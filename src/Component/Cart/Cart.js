...
const Cart = (props) => {
    ...
    return (
        <div className='card my-5 sticky-top'>
            ...
            <h5 className='text-danger'>"Choose only 4 guns."</h5>
            <div className='card-body'>
                {
                    selectedItem.map(product => <CartProduct product={product} key={product.id}></CartProduct>)
                }
            </div>
            <button className='btn btn-primary mx-4 my-2' onClick={() => luckyOne()}> <strong>Choose one</strong> </button>
            <button className='btn btn-danger mx-4 my-1' onClick={() => deleteCart()} > <strong>Delet all</strong> </button>
